...
export default function SubscriptionWorkspace() {
  ...
  return <div className="space-y-5">
    ...
    {loading ? <div className="space-y-3">{[1, 2, 3].map((item) => <div key={item} className="h-24 animate-pulse rounded-2xl border border-zinc-800 bg-zinc-900/50" />)}</div> : <div className="overflow-x-auto rounded-2xl border border-zinc-800 bg-zinc-900/55"><table className="w-full min-w-[900px] text-right text-sm"><thead className="border-b border-zinc-800 bg-zinc-950/70 text-zinc-400"><tr><th className="p-4">العميل</th><th className="p-4">الخدمة والباقـة</th><th className="p-4">الانتهاء</th><th className="p-4">القيمة</th><th className="p-4">الحالة</th><th className="p-4">إجراءات</th></tr></thead><tbody>{visible.map((item) => { const days = dayCount(item.endDate); const tone = item.status === 'active' ? 'bg-emerald-500/10 text-emerald-300' : item.status === 'canceled' ? 'bg-rose-500/10 text-rose-300' : 'bg-zinc-800 text-zinc-300'; return <tr key={item.id} className={`border-b border-zinc-800/70 last:border-0 ${selectedId === item.id ? 'bg-emerald-500/5' : ''}`}><td className="p-4"><button onClick={() => openDetails(item)} className="text-right"><p className="font-bold text-zinc-100 hover:text-emerald-300">{item.customer.name}</p><p className="mt-1 text-xs text-zinc-500" dir="ltr">{item.customer.phone}</p></button></td><td className="p-4"><button onClick={() => openDetails(item)} className="text-right"><p className="font-bold text-zinc-100 hover:text-emerald-300">{item.service.name}</p><p className="mt-1 text-xs text-zinc-400">{item.package || item.servicePlan?.name || 'الخطة الافتراضية'}</p></button></td><td className="p-4"><p className="text-zinc-200">{dateLabel(item.endDate)}</p><p className={`mt-1 text-xs ${days < 0 ? 'text-rose-300' : days <= 7 ? 'text-amber-200' : 'text-zinc-500'}`}>{days < 0 ? 'منتهي' : `${days} يوم متبقي`}</p></td><td className="p-4 font-black text-emerald-300">{Number(item.sellingPrice).toFixed(2)} EGP</td><td className="p-4"><span className={`rounded-lg px-2.5 py-1 text-xs font-black ${tone}`}>{item.status === 'active' ? 'نشط' : item.status === 'canceled' ? 'ملغي' : item.status}</span></td><td className="p-4"><button onClick={() => openDetails(item)} className="rounded-lg border border-emerald-500/30 px-3 py-2 text-xs font-bold text-emerald-300 hover:bg-emerald-500/10">فتح وإدارة</button></td></tr>; })}</tbody></table>{!visible.length ? <p className="p-12 text-center text-sm text-zinc-500">لا توجد اشتراكات مطابقة للبحث.</p> : null}</div>}

    {selected ? <section className="rounded-2xl border border-emerald-500/30 bg-zinc-900/75 p-5 shadow-2xl shadow-black/20">
      <div className="flex flex-col gap-4 border-b border-zinc-800 pb-5 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm font-bold text-emerald-400">تفاصيل الاشتراك</p><h3 className="mt-2 text-xl font-black">{selected.service.name} <span className="text-zinc-500">لـ {selected.customer.name}</span></h3><p className="mt-2 text-sm text-zinc-400">رقم الاشتراك: {selected.orderNo}</p></div><button onClick={() => setSelectedId(null)} className="inline-flex items-center gap-2 self-start rounded-xl border border-zinc-700 px-3 py-2 text-sm font-bold text-zinc-300 hover:bg-zinc-800"><X className="h-4 w-4" />إغلاق</button></div>
