...
function previewBackup(content: string): BackupPreview {
  const archive = JSON.parse(content) as { format?: string; version?: number; createdAt?: string; data?: Record<string, unknown> };
  if (archive.format !== 'nazzemly-data-backup' || archive.version !== 1 || !archive.data || typeof archive.data !== 'object') {
    throw new Error('اختر ملف نسخة احتياطية تم تنزيله من Nazzemly.');
  }
  const sections = Object.entries(sectionLabels).flatMap(([id, label]) => {
...
export default function DataTransferPage() {
  ...
  return <section dir="rtl" className="mx-auto max-w-5xl space-y-6 pb-12">
    <header className="border-b border-zinc-800 pb-6">
      ...
      <p className="mt-2 max-w-3xl text-sm leading-7 text-zinc-400">نزّل نسخة واحدة شاملة من بيانات متجرك، أو استورد نفس الملف من مكان واحد. لا يحذف الاستيراد أي بيانات موجودة.</p>
    </header>
    ...
    <section className="grid gap-4 lg:grid-cols-2">
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-6">
        ...
        <h2 className="mt-5 text-xl font-black text-white">تصدير كل بيانات المتجر</h2>
        <p className="mt-2 min-h-12 text-sm leading-6 text-zinc-400">ملف Nazzemly واحد يشمل العملاء والخدمات والاشتراكات والمصروفات والإعلانات.</p>
        <div className="mt-5 rounded-xl border border-zinc-800 bg-zinc-950/60 p-3 text-xs leading-6 text-zinc-400"><ShieldCheck className="ml-1 inline h-4 w-4 text-emerald-300" />التصدير للقراءة فقط؛ لا يغير أي بيانات في المتجر.</div>
        <button type="button" disabled={isPending} onClick={download} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 py-3 text-sm font-black text-zinc-950 transition-colors hover:bg-emerald-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300 disabled:opacity-50"><Download className="h-4 w-4" />{isPending ? 'جارٍ تجهيز الملف…' : 'تنزيل الملف الشامل'}</button>
      </div>

      <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-6">
        <span className="grid h-11 w-11 place-items-center rounded-xl bg-sky-500/10 text-sky-300"><FileUp className="h-5 w-5" /></span>
        <h2 className="mt-5 text-xl font-black text-white">استيراد ملف البيانات</h2>
        <p className="mt-2 min-h-12 text-sm leading-6 text-zinc-400">ارفع ملف Nazzemly الشامل نفسه. ستظهر معاينة للأقسام قبل بدء الاستيراد.</p>
    ...
    {preview ? <section className="rounded-2xl border border-sky-500/30 bg-sky-500/5 p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="flex items-center gap-2 text-lg font-black text-white"><FileCheck2 className="h-5 w-5 text-sky-300" />معاينة قبل الاستيراد</h2><p className="mt-1 text-sm text-sky-100/75">تاريخ إنشاء الملف: {formatDate(preview.createdAt)}</p></div><button type="button" disabled={isPending} onClick={importFile} className="inline-flex items-center gap-2 rounded-xl bg-emerald-500 px-4 py-3 text-sm font-black text-zinc-950 transition-colors hover:bg-emerald-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300 disabled:opacity-50"><Upload className="h-4 w-4" />{isPending ? 'جارٍ الاستيراد…' : 'بدء الاستيراد الآمن'}</button></div><div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{preview.sections.map((section) => <div key={section.id} className="rounded-xl border border-sky-500/15 bg-zinc-950/50 px-3 py-3"><p className="text-sm font-bold text-zinc-100">{section.label}</p><p className="mt-1 text-xs text-zinc-400">{section.rows.toLocaleString('ar-EG')} صف</p></div>)}</div><p className="mt-5 rounded-xl border border-amber-500/20 bg-amber-500/5 px-3 py-2.5 text-xs leading-6 text-amber-100">يضيف النظام البيانات الجديدة ويحدّث المتطابق منها وفق مفاتيحها الآمنة. لا يوجد حذف أو مسح تلقائي للبيانات الحالية.<HelpTip text="العملاء يطابقون برقم الهاتف، والخدمات باسم الخدمة، والاشتراكات ذات رقم الطلب المكرر لا تتكرر." /></p></section> : null}

    {lastResult ? <section className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5"><h2 className="text-lg font-black text-white">نتيجة الاستيراد</h2><div className="mt-4 divide-y divide-zinc-800">{lastResult.map((result) => <div key={result.dataSet} className="flex flex-wrap items-center justify-between gap-3 py-3 first:pt-0 last:pb-0"><div><p className="font-bold text-zinc-100">{sectionLabels[result.dataSet] || result.dataSet}</p><p className={`mt-1 text-xs ${result.success ? 'text-zinc-400' : 'text-rose-200'}`}>{result.success ? `${result.created} جديد · ${result.updated} محدث · ${result.skipped} تم تجاوزه` : result.error || 'تعذر الاستيراد'}</p></div>{result.success ? <CheckCircle2 className="h-5 w-5 text-emerald-300" /> : <X className="h-5 w-5 text-rose-300" />}</div>)}</div></section> : null}
  </section>;
}
